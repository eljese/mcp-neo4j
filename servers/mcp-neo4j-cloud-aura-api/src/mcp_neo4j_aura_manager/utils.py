...
def _validate_region(cloud_provider: str, region: str) -> None:
    ...
    if cloud_provider == "gcp" and region.count("-") != 1:
        raise ValueError(
            f"Invalid region for GCP: {region}. Must follow the format 'region-zonenumber'. Refer to https://neo4j.com/docs/aura/managing-instances/regions/ for valid regions."
        )
    elif cloud_provider == "aws" and region.count("-") != 2:
        raise ValueError(
            f"Invalid region for AWS: {region}. Must follow the format 'region-zone-number'. Refer to https://neo4j.com/docs/aura/managing-instances/regions/ for valid regions."
        )
    elif cloud_provider == "azure" and region.count("-") != 0:
        raise ValueError(
            f"Invalid region for Azure: {region}. Must follow the format 'regionzone'. Refer to https://neo4j.com/docs/aura/managing-instances/regions/ for valid regions."
        )
...
def parse_client_id(args: argparse.Namespace) -> str:
    ...
    if args.client_id is not None:
        ...
        if os.getenv("NEO4J_AURA_CLIENT_ID") is not None:
            ...
            logger.error(
                "Error: No Neo4j Aura Client ID provided. Please provide it as an argument or environment variable."
            )
            raise ValueError(
                "No Neo4j Aura Client ID provided. Please provide it as an argument or environment variable."
            )
...
def parse_client_secret(args: argparse.Namespace) -> str:
    ...
    if args.client_secret is not None:
        ...
        if os.getenv("NEO4J_AURA_CLIENT_SECRET") is not None:
            ...
            logger.error(
                "Error: No Neo4j Aura Client Secret provided. Please provide it as an argument or environment variable."
            )
            raise ValueError(
                "No Neo4j Aura Client Secret provided. Please provide it as an argument or environment variable."
            )
...
def parse_server_port(
    ...
    # check environment variable
        ...
        # if environment variable does not exist and using stdio transport
            ...
            logger.info(
                "Info: No server host provided and transport is `stdio`. `server_port` will be 0."
            )
            return 0
...
def parse_stateless(
    ...
    # check environment variable
        ...
        if env_stateless is not None:
            ...
            if transport == "stdio":
                logger.warning(
                    "Warning: Stateless mode provided, but transport is `stdio`. The `NEO4J_MCP_SERVER_STATELESS` environment variable will be set, but ignored."
                )
            elif stateless_bool:
...
def process_config(
    ...
    config: dict[str, str | int | list[str] | bool | None] = {}
    ...
    config["stateless"] = parse_stateless(
        args, cast(Literal["stdio", "http", "sse"], config["transport"])
    )
