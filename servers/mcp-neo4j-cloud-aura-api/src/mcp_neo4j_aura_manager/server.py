...
def create_mcp_server(aura_manager: AuraManager, namespace: str = "") -> FastMCP:
    ...
    async def create_instance(
        ...
        region: str = Field(
            "us-central1", description="Region for the instance (e.g., 'us-central1')"
        ),
        type: str = Field(
            "free-db",
        ...
        vector_optimized: bool = Field(
            False, description="Whether the instance is optimized for vector operations"
        ),
        cloud_provider: str = Field(
            "gcp", description="Cloud provider (gcp, aws, azure)"
        ),
        graph_analytics_plugin: bool = Field(
            False, description="Whether to enable the graph analytics plugin"
        ),
        source_instance_id: str | None = Field(
            None, description="ID of the source instance to clone from"
        ),
    ) -> dict:
    ...
    @mcp.tool(
        name=namespace_prefix + "update_instance_vector_optimization",
        annotations=ToolAnnotations(
    ...
    async def update_instance_vector_optimization(
        instance_id: str, vector_optimized: bool
    ) -> dict:
    ...
    @mcp.tool(
        name=namespace_prefix + "delete_instance",
        annotations=ToolAnnotations(
